...
import de.tum.in.opcua.server.core.ClientInfo;
import de.tum.in.opcua.server.core.Session;
import de.tum.in.opcua.server.core.SessionManager;
import de.tum.in.opcua.server.core.UAServerException;
import de.tum.in.opcua.server.core.util.ArrayUtils;

public class SessionServiceHandler extends ServiceHandlerBase implements
	...
	public void onActivateSession(
		...
		final ActivateSessionRequest req = serviceReq.getRequest();
		final ActivateSessionResponse resp = new ActivateSessionResponse();

		final NodeId authToken = req.getRequestHeader()
				.getAuthenticationToken();
		final Session session = server.getSessionManager()
				.getSession(authToken);
		if (session == null) {
			resp.setResponseHeader(buildErrRespHeader(req,
					StatusCodes.Bad_SessionIdInvalid));
			resp.setResults(new StatusCode[] { new StatusCode(
					StatusCodes.Bad_SessionIdInvalid) });
			sendResp(serviceReq, resp);
			return;
		}

		LOG.info("clients secure channel-id: "
				+ serviceReq.getChannel().getSecureChannelId());
		LOG.info("security policy "
				+ serviceReq.getChannel().getSecurityPolicy());
		LOG.info("security mode "
				+ serviceReq.getChannel().getMessageSecurityMode());

		final ExtensionObject oToken = req.getUserIdentityToken();
		ClientIdentity clientIdentity = null;
		if (oToken != null) {
			try {
				final UserIdentityToken uToken = (UserIdentityToken) oToken
						.decode();

				/*
				 * validate if this policy is really supported by our server
				 */
				// TODO check why the UaCTT sends 0 for anonymous sessions and
				// not the policyID:
				...
				if (uToken instanceof UserNameIdentityToken) {
					final UserNameIdentityToken userNameToken = (UserNameIdentityToken) uToken;
					try {
						clientIdentity = readClientIdentity(userNameToken,
								session);
					...
					final AnonymousIdentityToken anonymToken = (AnonymousIdentityToken) uToken;
					LOG.info("user authenticates anonymously");

				} else if (uToken instanceof X509IdentityToken) {
					@SuppressWarnings("unused")
					final X509IdentityToken certToken = (X509IdentityToken) uToken;
		...
		if (clientIdentity != null) {
			final boolean authenticated = server.authenticate(clientIdentity);
			if (authenticated) {
		...
		final ResponseHeader respHeader = buildRespHeader(req);
		 ...
		 */
		final byte[] nonce = CryptoUtil.createNonce(NONCE_LENGTH);
		resp.setServerNonce(nonce);
		session.setLastNonce(nonce);

		resp.setResults(new StatusCode[] { StatusCode.GOOD });
		LOG.debug("client sent software certificates:"
				+ req.getClientSoftwareCertificates());
	...
	public void onCancel(
		...
		initRequestContext(serviceReq);
		final CancelResponse resp = new CancelResponse();
		final CancelRequest req = serviceReq.getRequest();
	...
	public void onCloseSession(
		...
		initRequestContext(serviceReq);
		final CloseSessionRequest req = serviceReq.getRequest();
		...
		/*
		 * if (req.getDeleteSubscriptions()) { // delete subscriptions } else {
		...
		final CloseSessionResponse resp = new CloseSessionResponse(
				buildRespHeader(req));
		sendResp(serviceReq, resp);
	}
	...
	public void onCreateSession(
		...
		final CreateSessionRequest req = serviceReq.getRequest();
		final CreateSessionResponse resp = new CreateSessionResponse();
		...
		final SessionManager sessionMgr = getSessionManager();

		final Session session = sessionMgr.createSession();
		...
		try {
			if (req.getClientCertificate() != null) {
				clientInfo.setClientCertificate(new Cert(req
		...
		final NodeId sessionID = NodeId.randomGUID(1);
		session.setSessionID(sessionID);
		final NodeId authToken = NodeId.randomGUID(1);
		...
		final byte[] serverCert = server.getStackServer()
				.getApplicationInstanceCertificate().getCertificate()
				.getEncoded();
		...
		try {
			if (req.getClientCertificate() != null) {
				resp.setServerSignature(getServerSignature(serviceReq
						.getChannel().getSecurityPolicy(), ArrayUtils.concat(
						req.getClientCertificate(), req.getClientNonce())));
			}
		} catch (final Exception e) {
			LOG.error(e.getMessage(), e);
		}
		...
		final List<EndpointDescription> endpointList = server
				.getEndpointDescriptionsForUri(req.getEndpointUrl());
		LOG.info("found endpoints: " + endpointList.size());
		if (endpointList.size() > 0) {
			final EndpointDescription[] endpointArray = new EndpointDescription[endpointList
					.size()];
			endpointList.toArray(endpointArray);
			resp.setServerEndpoints(endpointArray);
		}
	...
	private SignatureData getServerSignature(SecurityPolicy secPolicy,
			byte[] dataToSign) throws InvalidKeyException, SignatureException,
			NoSuchAlgorithmException {
		SignatureData signature = null;

		final PrivKey key = server.getStackServer()
				.getApplicationInstanceCertificate().getPrivateKey();
		signature = CertificateUtils.sign(key.getPrivateKey(),
				secPolicy.getAsymmetricSignatureAlgorithmUri(), dataToSign);
	...
	private Double getSessionTimeout(Double requestedTimeout) {
		...
		if (requestedTimeout != null
				&& (requestedTimeout >= 1000 * 60 * 10 || requestedTimeout <= 1000 * 60 * 60)) {
	...
	private void validateUserPolicy(String policyId) throws UAServerException {
		if (!server.supportsUserTokenPolicy(policyId)) {
			// TODO use this line of code. it was commented out because of the
			// UaCtt
			// throw new
			// UAServerException("server does not support UserTokenPolicy with id "
			// + policyId);
		}
	}
	 ...
	 */
	private ClientIdentity readClientIdentity(
			UserNameIdentityToken userNameToken, Session session)
			throws UAServerException {
		LOG.info("client uses username+password to authenticate, policy-id is "
				+ userNameToken.getPolicyId());

		final String user = userNameToken.getUserName();
		...
		if (userNameToken.getPassword() != null) {
			...
			if (userNameToken.getEncryptionAlgorithm() != null) {
				...
				final String encryptAlgo = userNameToken
						.getEncryptionAlgorithm();
				encryptPasswd = userNameToken.getPassword();

				LOG.info(String.format("used encryption algorithm: %s",
						encryptAlgo));
				...
				if (encryptPasswd != null) {
					...
					final KeyPair kp = server.getStackServer()
							.getApplicationInstanceCertificate();

					byte[] decryptedBytes;
					try {
						final Cipher cipher = CryptoUtil
								.getAsymmetricCipher(encryptAlgo);
						cipher.init(Cipher.DECRYPT_MODE, kp.getPrivateKey()
								.getPrivateKey());
					 ...
					 * length NONCE_LENGTH
					 */
					final byte[] passwdBytes = Arrays.copyOfRange(
							decryptedBytes, 4, decryptedBytes.length
									- NONCE_LENGTH);
					final byte[] lastNonceBytes = Arrays.copyOfRange(
							decryptedBytes, decryptedBytes.length
									- NONCE_LENGTH, decryptedBytes.length);

					password = new String(passwdBytes);

					// test if the client sent the same nonce in his request
					// as the server in his last response.
					final boolean nonceValid = Arrays.equals(lastNonceBytes,
