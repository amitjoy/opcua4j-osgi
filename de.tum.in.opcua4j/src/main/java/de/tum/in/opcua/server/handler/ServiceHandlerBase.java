...
public class ServiceHandlerBase {
	...
	public static ResponseHeader buildRespHeader(ServiceRequest req) {
		...
		final ResponseHeader respHeader = new ResponseHeader();
	 ...
	 * @return
	 */
	public static ResponseHeader buildErrRespHeader(ServiceRequest req,
			UnsignedInteger errCode) {
		final ResponseHeader respHeader = new ResponseHeader();
	 ...
	 * @param resp
	 */
	protected <T extends ServiceResponse> void sendResp(
			EndpointServiceRequest<? extends ServiceRequest, T> req, T resp) {
	...
	/**
	 * fills the {@link RequestContext} object with common information like the
	 * associated session and the given {@link ServiceRequest};
	 * 
	 * @param serviceReq
	 */
	protected void initRequestContext(
			EndpointServiceRequest<? extends ServiceRequest, ? extends ServiceResponse> req) {
		final RequestContext ctx = RequestContext.get();

		final NodeId authToken = req.getRequest().getRequestHeader()
	...
	protected Session getSession(ServiceRequest req) {
		return server.getSessionManager().getSession(
				req.getRequestHeader().getAuthenticationToken());
	}
