...
import de.tum.in.opcua.server.core.history.HistoryManagerAdapter;
import de.tum.in.opcua.server.core.history.IHistoryManager;
import de.tum.in.opcua.server.core.parse.CsvNodeIDParser;
import de.tum.in.opcua.server.core.parse.ParsedElement;
import de.tum.in.opcua.server.core.parse.XmlModelParser;
import de.tum.in.opcua.server.core.util.FileUtils;
import de.tum.in.opcua.server.core.util.NodeUtils;

/**
 * manages all Nodes of the standard opc ua information model in the
 * http://opcfoundation.org/UA/ namespace. he parses the standard nodes from an
 * XML and stores them in a map for accessing.
 * 
 * @author harald
 *
 */
public class CoreNodeManager implements INodeManager {
	...
	public void init(AddressSpace addrSpace, int nsIndex) {
		...
		final INodeIDs nodeIds = new CsvNodeIDParser(
				FileUtils.getFileFromResource2(CSV_FILE_PATH));

		final XmlModelParser modelParser = new XmlModelParser(
				FileUtils.getFileFromResource2(XML_FILE_PATH), nodeIds);

		// parse the xml file
		final List<ParsedElement> parsedElements = modelParser.parseNodes();

		LOG.debug("parsed elements " + parsedElements.size());

		/*
		 * add all parsed elements (nodes and references) to our nodes-map. all
		 * nodes are added at the beginning because references can only be added
		 * to existing nodes in our map.
		 */
		for (final ParsedElement pe : parsedElements) {
	...
	public void addReferences(List<AddReferencesItem> refItems,
			boolean bidirectional) {
		for (final AddReferencesItem refItem : refItems) {
			addReference(refItem, bidirectional);
		}
	}

	public void addReference(AddReferencesItem refItem, boolean bidirectional) {
		addReference(refItem.getSourceNodeId(),
				NodeUtils.toNodeId(refItem.getTargetNodeId()),
				refItem.getReferenceTypeId(), refItem.getIsForward(),
				bidirectional);
	}

	public void addReference(NodeId srcNode, NodeId targetNode,
			NodeId referenceTypeId, boolean isForward) {

		/*
		 * TODO insert inverse references to have bidirectional references. for
		 * example a node with hasTypedefinition reference is one direction.
		 * it's type node would then have a reference TypeDefinitionOf (which is
		 * the inverseName, hence inverse=true) to the first one.
		 */

		final ReferenceNode newRef = NodeFactory.getReferenceNodeInstance(
				referenceTypeId, !isForward, targetNode);
		NodeUtils.addReferenceToNode(getNode(srcNode), newRef);
	}

	public void addReference(NodeId srcNode, NodeId targetNode,
			NodeId referenceTypeId, boolean isForward, boolean bidirectional) {
	...
	public Node getNode(NodeId nodeId) {
		...
		if (nodeId.equals(Identifiers.Server_NamespaceArray)) {
			...
			final ServerStatusDataType serverState = new ServerStatusDataType();
