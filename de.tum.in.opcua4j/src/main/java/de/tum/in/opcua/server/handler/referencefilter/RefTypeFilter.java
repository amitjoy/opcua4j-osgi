package de.tum.in.opcua.server.handler.referencefilter;
...
public class RefTypeFilter implements IReferenceFilter {
	...
	private static Set<NodeId> hierarchicalRefs = new HashSet<NodeId>(
			Arrays.asList(Identifiers.HierarchicalReferences,
					Identifiers.HasChild, Identifiers.Aggregates,
					Identifiers.HasComponent, Identifiers.HasOrderedComponent,
					Identifiers.HasProperty, Identifiers.HasSubtype,
					Identifiers.HasEventSource, Identifiers.HasNotifier,
					Identifiers.Organizes));

	@Override
	public List<ReferenceDescription> filter(
			List<ReferenceDescription> refsToFilter,
			BrowseDescription browseDesc) {
		List<ReferenceDescription> filteredReferences = new ArrayList<ReferenceDescription>();

		final NodeId refType = browseDesc.getReferenceTypeId();
		if (refType == null || NodeId.ZERO.equals(refType)
				|| Identifiers.References.equals(refType)) {
			// return all nodes, no filtering
			filteredReferences = refsToFilter;
		} else {
			final boolean inclSubtypes = browseDesc.getIncludeSubtypes() == null ? false
					: browseDesc.getIncludeSubtypes().booleanValue();
			for (final ReferenceDescription refDesc : refsToFilter) {
				if (inclSubtypes) {
					...
					if (Identifiers.HierarchicalReferences.equals(refType)) {
						...
						if (hierarchicalRefs.contains(refDesc
								.getReferenceTypeId())) {
