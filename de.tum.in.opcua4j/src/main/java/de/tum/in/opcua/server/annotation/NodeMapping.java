...
/**
 * contains fields which are used to create node instances for a specific
 * java-class.
 * 
 * @author harald
 *
...
public class NodeMapping {
	...
	private Class<?> clazz;
	/**
	 * the name which is used as ID in a node for the associated java class
	 * (clazz field)
	 */
	...
	/**
	 * type definition created for this nodemapping (and the underlying
	 * javaClass). whenever a node is created by this Nodemapping, his
	 * HasTypeDefinition relationship points to this {@link ExpandedNodeId}.
	 */
	...
	/**
	 * field which contains the variables/properties value. this is only used
	 * for Variable-Nodes. one variable-node can only have one value
	 */
	 ...
	 */
	public NodeMapping(Class<?> clazz, NodeClass nodeClass, Field idField,
			Field displNameField, Field descField,
			Map<String, ReferenceMapping> referencesByName) {
		this.clazz = clazz;
		this.nodeName = clazz.getSimpleName(); // for now its the name of the
												// class. later it can be
												// declared with @UaNode
												// annotation on the Java Type.
		this.nodeClass = nodeClass;
		this.idField = idField;
		this.displNameField = displNameField;
		this.descField = descField;
		this.referencesByName = referencesByName != null ? referencesByName
				: new HashMap<String, ReferenceMapping>();
	}
	 ...
	 *            the referencesByName to set
	 */
	public void setReferencesByName(
			Map<String, ReferenceMapping> referencesByName) {
	...
	public Object readIdField(Object obj) throws IllegalArgumentException,
			IllegalAccessException {
		return readValue(idField, obj);
	}

	public String readDisplNameField(Object obj)
			throws IllegalArgumentException, IllegalAccessException {
		return readValue(displNameField, obj).toString();
	}

	public String readDescField(Object obj) throws IllegalArgumentException,
			IllegalAccessException {
		return readValue(descField, obj).toString();
	}

	public Object readValueField(Object obj) throws IllegalArgumentException,
			IllegalAccessException {
		return readValue(valueField, obj);
	}

	public Object readProperty(String name, Object obj)
			throws IllegalArgumentException, IllegalAccessException {
		return readValue(referencesByName.get(name).getField(), obj);
	}

	/**
	 * reads the value from the given Objects field with the given name. if the
	 * field is of an built-in type, the value is read directly. <br/>
	 * <br/>
	 * the field can also be some custom object, where a single property of the
	 * custom object has to be annotated with {@link Value}. the value of the
	 * {@link Value}-annotated field in the custom object is returned in that
	 * case.
	 * 
	 * @param name
	 * @param obj
	...
	public Class<?> getReferencedDataType(String name) {
		Class<?> type = null;
		final Field field = referencesByName.get(name).getField();
		if (field != null) {
			type = field.getType();
		}
		return type;
	...
	/**
	 * returns an ReferenceMapping for a given fieldName. The field should be
	 * annotated with any annotation which defines references like @Property or @Reference
	 * 
	 * @param fieldName
	 * @return
	 */
