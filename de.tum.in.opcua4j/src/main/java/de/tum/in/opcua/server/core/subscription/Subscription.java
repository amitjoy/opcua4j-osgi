...
public class Subscription {
	...
	/**
	 * cyclic rate in milliseconds that the Subscription returns Notifications
	 * to the Client
	 */
	 ...
	 * be a mimimum of three times the keep keep-alive count. When the
	 * publishing timer has expired this number of times without a Publish
	 * request being available to send a NotificationMessage, then the
	 * Subscription shall be deleted by the Server.
	 */
	private int lifetimeCount;

	/**
	 * When the publishing timer has expired this number of times without
	 * requiring any NotificationMessage to be sent, the Subscription sends a
	 * keep-alive Message to the Client.
	 */
	private int maxKeepAliveCount;

	/**
	 * The maximum number of notifications that the Client wishes to receive in
	 * a single Publish response. A value of zero indicates that there is no
	 * limit.
	 */
	 ...
	 * Subscription needs to send Notifications, the Server should dequeue a
	 * Publish request to the Subscription with the highest priority number. For
	 * Subscriptions with equal priority the Server should dequeue Publish
	 * requests in a round-robin fashion. When the keep-alive period expires for
	 * a Subscription it shall take precedence regardless of its priority, in
	 * order to prevent the Subscription from expiring. A Client that does not
	 * require special priority settings should set this value to zero.
	 * 
	 ...
	 *            the maxNotificationsPerPublish to set
	 */
