 ...
 * {@link INodeManager}s before the {@link AddressSpace} object is built. the
 * {@link INodeManager#init(AddressSpace)} method is called in exact the insert
 * order. means if one manager is inserted after another, the second one can
 * already make use of the first one in its own init-method.
 *
 * @author harald
 *
 */
public class AddrSpaceBuilder {
	...
	private final Map<Integer, INodeManager> managers;
	...
	public AddressSpace build() {
		// we call on every manager the init method and add it to the addrSpace
		// afterwards
		for (final Integer nsIndex : managers.keySet()) {
			final INodeManager manager = managers.get(nsIndex);
