...
public class NodeUtils {
	...
	public static void addReferenceToNode(Node node, ReferenceNode refNode) {
		final ReferenceNode[] newRefNode = new ReferenceNode[] { refNode };
		if (node != null) {
			if (node.getReferences() == null) {
				node.setReferences(newRefNode);
			} else {
				node.setReferences(ArrayUtils.concat(node.getReferences(),
						newRefNode));
			}
		}
	}

	public static void addReferenceListToNode(Node node,
			List<ReferenceNode> refNodeList) {
		final ReferenceNode[] newRefNode = refNodeList
				.toArray(new ReferenceNode[refNodeList.size()]);
		if (node.getReferences() == null) {
			node.setReferences(newRefNode);
		} else {
			node.setReferences(ArrayUtils.concat(node.getReferences(),
	...
	public static ReferenceDescription mapReferenceNodeToDesc(
			ReferenceNode refNode, Node targetNode) {
		ReferenceDescription refDesc;

		refDesc = getRefDescForNode(targetNode, refNode.getReferenceTypeId(),
				!refNode.getIsInverse());

		return refDesc;
	}

	public static ReferenceDescription getRefDescForNode(Node targetNode,
			NodeId referenceType, boolean isForward) {
		final ReferenceDescription refDesc = new ReferenceDescription();
		...
		if (node != null && node.getReferences() != null) {
			for (final ReferenceNode refNode : node.getReferences()) {
				if (!refNode.getIsInverse()
						&& Identifiers.HasTypeDefinition.equals(refNode
								.getReferenceTypeId())) {
					typeDef = refNode.getTargetId();
					LOG.info("found typedefinition " + typeDef + " for node "
							+ node.getNodeId());
					break;
				}
			}
	...
	/**
	 * returns the value rank for the given class. remember: -3: scalar or one
	 * dimensional array -2: any, i.e. scalar or array with any dimension -1:
	 * scalar 0: array with one or more dimensions 1: array with one dimension
