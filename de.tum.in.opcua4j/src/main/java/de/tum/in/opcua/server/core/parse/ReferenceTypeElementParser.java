...
public class ReferenceTypeElementParser extends AbstractElementParser {
	...
	public ParsedElement parseElement(Element elem, INodeIDs nodeIds,
		...
		if (nodeId != null) {
			...
			final boolean isSymmetric = readIsSymmetric();
			final String inverseName = readInverseName();

			final Node newNode = NodeFactory.getReferenceTypeNodeInstance(
					browseName, "description goes here", browseName, locale,
					nodeId, isAbstract, isSymmetric, inverseName);
