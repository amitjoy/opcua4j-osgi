...
/**
 * this class decorates anny {@link INodeIDs} so that is always prefixes the
 * given prefix to the desired node-name when a client uses
 * {@link INodeIDs#getNodeIdByName(String)}. it can be set so that a client can
 * transparently read a subset of {@link NodeId}s. <br />
 * <br />
 * for example when parsing an xml file the child-elements often have their
 * parents name prefixed. <br />
 * example: the objecttype "ServerType" has several children like "ServerArray"
 * but the "ServerArray"s NodeId is accessed by "ServerType_ServerArray", hence
 * "ServerType_" would be a useful prefix in that case.
 */
...
public class NodeIDsPrefixDecorator implements INodeIDs {
	...
	public NodeIDsPrefixDecorator(String readPrefix, INodeIDs nodeIDs) {
		...
		if (nodeIDs instanceof NodeIDsPrefixDecorator) {
			final NodeIDsPrefixDecorator innerOne = (NodeIDsPrefixDecorator) nodeIDs;

			/*
			 * if the given one is another prefix decorator, we use his INodeIDs
			 * implementation and append his prefix to our one
			 */
	...
	public NodeId getNodeIdByName(String name) {
		...
		// if the given name does already begin with the prefix, we do not add
		// it a second time!
